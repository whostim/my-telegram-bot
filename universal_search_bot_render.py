...
"""
        ...
        async def quick_search_handler(message: types.Message):
            ...
            self.user_states[message.from_user.id] = 'quick'

        @self.dp.message()
        async def handle_all_messages(message: types.Message):
            user_id = message.from_user.id
            search_type = self.user_states.get(user_id)
            
            if not search_type or message.text.startswith('/'):
                return
    ...
    async def stop(self):
        ...
        try:
            ...
            logger.info("✅ Поисковик закрыт")
        except Exception as e:
            logger.error(f"❌ Ошибка закрытия поисковика: {e}")

# ===== ГЛАВНАЯ ФУНКЦИЯ С AUTORESTART =====
async def main():
    ...
    try:
        ...
        health_server = RenderHealthServer()
...
if __name__ == "__main__":
    logger.info("🎯 Запуск бота для Render с авто-деплоем")
    
    # Простой запуск - Render сам перезапустит при необходимости
    try:
        asyncio.run(main())
    except Exception as e:
        logger.error(f"💥 Фатальная ошибка: {e}")
        # Выход с ненулевым кодом заставит Render перезапустить сервис
        sys.exit(1)
